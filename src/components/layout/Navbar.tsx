...
import { Menu, Sun, Moon, Download, FileJson, FileText, Shield, Eye } from "lucide-react";
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 h-14">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="hidden sm:flex items-center gap-0.5 px-1 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800">
            <button onClick={() => setRole("viewer")}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${
                role === "viewer" ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm"
            ...
            <button onClick={() => setRole("admin")}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${
                role === "admin" ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm"
                  : "text-slate-500 dark:text-slate-400"
              }`}>
          ...
          <div className="relative" ref={exportRef}>
            <button onClick={() => setExportOpen(!exportOpen)} className="btn-ghost">
            ...
            {exportOpen && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden z-50">
                <button onClick={() => { exportToCSV(transactions); setExportOpen(false); }}
                ...
                <button onClick={() => { exportToJSON(transactions); setExportOpen(false); }}
                  className="flex items-center gap-2.5 w-full px-3 py-2.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
