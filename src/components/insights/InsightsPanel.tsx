...
import { PiggyBank, TrendingUp, BarChart3, Calendar, AlertTriangle, Receipt, ArrowUpRight, ArrowDownRight } from "lucide-react";
import type { Insight } from "../../types";
import { useFinanceStore } from "../../store/useFinanceStore";

const ICON_MAP: Record<string, React.ElementType> = {
  "piggy-bank": PiggyBank, "trending-up": TrendingUp, "bar-chart": BarChart3,
  calendar: Calendar, "alert-triangle": AlertTriangle, receipt: Receipt,
};
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-start gap-3">
        <div className={`p-2 rounded-lg flex-shrink-0 ${TYPE_ICON_STYLES[insight.type]}`}>
          <Icon className="w-4 h-4" />
        </div>
        <div className="min-w-0">
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{insight.title}</p>
          <div className="flex items-center gap-1 mt-0.5">
            <p className="text-sm text-slate-500 dark:text-slate-400">{insight.value}</p>
...
export default function InsightsPanel() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Insights</h3>
